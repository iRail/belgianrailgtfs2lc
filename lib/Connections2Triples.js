...
Connections2Triples.prototype._transform = function (connection, encoding, done) {
  //URI strategy for the iRail GTFS
  var id = this._baseUris.connections + encodeURIComponent(connection.departureStop + '/' + connection.departureTime.toISOString().substr(0,10) + '/' + connection.route);
  this.push({
    subject : id,
