...
URIStrategy.prototype.getId = function (connection) {
  ...
  connection.departureTime = moment(connection.departureTime).tz('Europe/Brussels')
  return this._baseUris.connections + encodeURIComponent(connection.departureStop) + '/' + encodeURIComponent(connection.departureTime.format().substr(0,10).replace(/[-:]/g,'')) + '/' + encodeURIComponent(connection.route);
};
