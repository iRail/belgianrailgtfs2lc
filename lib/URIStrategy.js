...
URIStrategy.prototype.getId = function (connection) {
  ...
  return this._baseUris.connections + encodeURIComponent(connection.departureStop) + '/' + encodeURIComponent(connection.departureTime.toISOString().substr(0,16).replace(/[-:]/g,'')) + '/' + encodeURIComponent(connection.route);
};
